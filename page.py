...
class Page():
    def __init__(
        ...
        # Set Slug of Page
        if not any([slug, getattr(self,'slug', False)]):
            slug = getattr(self, 'name', None) \
                or getattr(self, 'id', None) \
                or getattr(self, 'content_path', 'unnamed_file')
            self.slug = slug
        ...
        if not getattr(self, 'url', None):
            self.url = Path(self.url_root).joinpath(
                    Path(self.slug).with_suffix(url_suffix))
