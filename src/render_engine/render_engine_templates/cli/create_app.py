...
def create_templates_folder(
    *,
    ...
    templates_path = pathlib.Path.mkdir(
        pathlib.Path(templates_folder_name),
        exist_ok=exists_ok,
    )
    content = templates_path.joinpath(index_path).write_text(
        environment.get_template("create_app_templates")
    )
...
def typer_app(
    ...
    for template in templates:
        pathlib.Path(templates_path).joinpath(template).write_text(
            environment.get_template("base_collection_path" / template).render()
        )
